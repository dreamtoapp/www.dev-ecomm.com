...
const getLocation = async (options: GeolocationOptions = {}): Promise<GeolocationResponse> => {
  ...
  for (let attempt = 0; attempt < (finalOptions.retries || 0); attempt++) {
    ...
    if (result.coords) {
      ...
      if (result.coords.accuracy < 50) { // Good enough accuracy
        return result;
      }
    }
  ...
  if (!bestResult?.coords) {
    ...
    return ipResult.coords ? ipResult : {
      ...ipResult,
...
const displayLocation = async () => {
  ...
  if (result.coords) {
    console.log(`Obtained ${result.source.toUpperCase()} location:
  ...
  try {
    ...
    console.log('Comparison Results:', {
      ...
      latDifference: ourLocation.coords?.latitude ? 
        ourLocation.coords.latitude - theirData.lat : null,
      lngDifference: ourLocation.coords?.longitude ? 
        ourLocation.coords.longitude - theirData.lng : null
    });
  } catch (error) {
