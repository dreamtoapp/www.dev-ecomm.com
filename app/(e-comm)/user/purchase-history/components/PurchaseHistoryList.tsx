...
export default function PurchaseHistoryList({ purchases }: PurchaseHistoryListProps) {
  ...
  const handleRateProduct = (product: { id: string; name: string; slug?: string; imageUrl: string }) => {
      ...
      <Accordion type="single" collapsible className="w-full">
        {purchases.map((purchase) => (
          <AccordionItem key={purchase.orderId} value={purchase.orderId}>
            <AccordionTrigger className="px-4 py-3 bg-card hover:bg-card/80 rounded-lg">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between w-full text-right gap-2">
                <div>
                  <span className="font-medium">طلب #{purchase.orderNumber}</span>
                  <span className="text-sm text-muted-foreground block sm:inline sm:mr-2">
                    {format(new Date(purchase.orderDate), "d MMMM yyyy", { locale: ar })}
                  </span>
                </div>
                <Badge
                  variant="outline"
                  className={cn("mr-auto", getStatusColor(purchase.status))}
                >
            ...
            <AccordionContent className="px-4 pt-4">
              <div className="space-y-4">
                {purchase.products.map((product) => (
                  <div key={product.id} className="flex flex-col sm:flex-row gap-4 pb-4">
