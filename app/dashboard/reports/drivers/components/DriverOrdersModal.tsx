...
export function DriverOrdersModal({ driverId, driverName }: DriverOrdersModalProps) {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    fetchDriverOrders(driverId, page, pageSize).then((data) => {
      if (mounted) {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-gray-400">جاري التحميل...</TableCell>
              </TableRow>
            ) : orders.length === 0 ? (
              <TableRow>
              ...
              <TableRow key={order.id}>
                ...
                <TableCell>{order.amount?.toLocaleString('ar-EG', { minimumFractionDigits: 2 })} ر.س</TableCell>
                <TableCell>{order.customer?.name || '-'}</TableCell>
                <TableCell>{order.createdAt ? new Date(order.createdAt).toLocaleString('ar-EG') : '-'}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
